...
if ($khachhang_id==0) {
	phpAlertUrl("Please login to continue!","../backshop/index.php");
}
...
if(isset($_POST['themgiohang'])){
	...
	$amount = $soluong * $gia;
 	...
 	if($count>0){
 		...
 		$soluong = $soluong;
 		$sql_giohang = "INSERT INTO tbl_giohang(tensanpham,sanpham_id,giasanpham,hinhanh,soluong,amount,khachhang_id) values ('$tensanpham',$sanpham_id,'$gia','$hinhanh',$soluong,$amount,$khachhang_id)";
	...
	if ($khachhang_id>0) {
 	...
 	if($sql_khachhang){
 		...
 		for($i=0;$i<count($_POST['thanhtoan_product_id']);$i++){
	 		...
	 		$sql_donhang = mysqli_query($con,"INSERT INTO tbl_donhang(sanpham_id,khachhang_id,soluong,mahang,amount) values ('$sanpham_id','$khachhang_id','$soluong','$mahang','$amount')");
	 		$sql_giaodich = mysqli_query($con,"INSERT INTO tbl_giaodich(sanpham_id,soluong,magiaodich,khachhang_id,amount) values ('$sanpham_id','$soluong','$mahang','$khachhang_id','$amount')");
	...
	$amount=0;
 	for($i=0;$i<count($_POST['thanhtoan_product_id']);$i++){
	 		$sanpham_id = $_POST['thanhtoan_product_id'][$i];
	 		$soluong = $_POST['thanhtoan_soluong'][$i];
	 		$sql_donhang = mysqli_query($con,"INSERT INTO tbl_donhang(sanpham_id,soluong,mahang,khachhang_id,tinhtrang,huydon,amount) values ('$sanpham_id','$soluong','$mahang','$khachhang_id','$tinhtrang','$huydon','$amount')");
	 		$sql_giaodich = mysqli_query($con,"INSERT INTO tbl_giaodich(sanpham_id,soluong,magiaodich,khachhang_id,tinhtrangdon,huydon,amount) values ('$sanpham_id','$soluong','$mahang','$khachhang_id','$tinhtrang','$huydon','$amount')");
	 		//$sql_delete_thanhtoan = mysqli_query($con,"DELETE FROM tbl_giohang WHERE sanpham_id='$sanpham_id' AND khachhang_id=$khachhang_id");
 		}
	...
	<div class="privacy py-sm-5 py-4">
		<div class="container py-xl-4 py-lg-2">
			...
			<h3 class="tittle-w3l text-center mb-lg-5 mb-sm-4 mb-3">
				...
				<?php 
				if(isset($_SESSION['dangnhap_home'])){
					echo '<p style="color:#000;">Hello: '.$_SESSION['dangnhap_home'].'<a href="index.php?quanly=logout">  Logout</a></p>';
					}if(isset($_GET['login'])){
						$dangxuat = $_GET['login'];
					}else{
						$dangxuat = '';
					...
					if($dangxuat=='dangxuat'){
						session_destroy();
						header('Location: ../backshop/index.php');
				}
				?>
						...
						?>
							<tr class="rem1">
								...
								<td class="invert-image">
									<a href="single.html">
										<img src="image/<?php echo $row_fetch_giohang['hinhanh'] ?>" alt=" "  width="200" height="200" class="img-responsive">
									</a>
								</td>
								<td class="invert">
									<input type="hidden" name="product_id[]" value="<?php echo $row_fetch_giohang['sanpham_id'] ?>">
									<input type="hidden" name="giasanpham[]" value="<?php echo $row_fetch_giohang['giasanpham'] ?>">
							...
							?>
							<tr>
								<td colspan="7">Total Amount : <?php echo number_format($total).'vnđ' ?></td>

							</tr>
							<tr>
								<td colspan="7"><input type="submit" class="btn btn-success" value="Update purchase" name="capnhatsoluong">
								<?php 
								...
								?>
								<input type="submit" class="btn btn-primary" value="Payment" name="thanhtoandangnhap">
			...
			<div class="checkout-left">
				<div class="address_form_agile mt-sm-5 mt-4">
					<h4 class="mb-sm-4 mb-3">Add address for Delivery</h4>
					<form action="payment.php" method="post" class="creditly-card-form agileinfo_form">
						<div class="creditly-wrapper wthree, w3_agileits_wrapper">
							<div class="information-wrapper">
								<div class="first-row">
									<div class="controls form-group">
										<input class="billing-address-name form-control" type="text" name="name" placeholder="Your name" required="">
									</div>
									...
									<div class="controls form-group">
										<select class="option-w3ls" name="giaohang">
											<option>Select option for payment</option>
								...
								?>
								<input type="submit" name="thanhtoan" class="btn btn-success" style="width: 20%" value="payment">
